import { fetchItems, validateItem, upsertItems, findLocalizationList, deleteItemGhosts, } from "#data";
import { LocalizationNamespace } from "#types";
const extractRawObjects = async (rawJson, key, rawObjects = []) => {
...
export const updateItems = async (version, onSuccess, onError) => {
    const rawItemsJson = await fetchItems(version);
    const rawItems = await extractRawObjects(rawItemsJson, "@uniquename");
    const localizationList = await findLocalizationList(rawItems.map((rawItem) => {
        return {
            namespace: LocalizationNamespace.ALBION_ONLINE,
            key: `@ITEMS_${rawItem["@uniquename"]}`,
        };
    }));
    const localizationMap = new Map(localizationList.map((localizationDocument) => [
    ...
    for (const rawItem of rawItems) {
        ...
        const localizationDocument = localizationMap.get(uniqueName);
        const enchantments = [];
        if ("enchantments" in rawItem) {
            enchantments.push(0);
            const rawEnchantments = await extractRawObjects(rawItem.enchantments.enchantment, "@enchantmentlevel");
